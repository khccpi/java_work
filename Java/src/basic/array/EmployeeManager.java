	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			if(menu == 1) { ////// 사원 정보 신규 등록 //////
				...
				outer1:for(int i=0; i<userNums.length; i++) {
				...
				outer:for(int i=0; i<userNums.length; i++) {
					System.out.println("'그만'을 작성하시면 조회를 중단합니다.");
					...
					if(empNum.equals(userNums[i])) {
						...
						outer:while(true) {
							...
							case 2: // 부서 변경
								...
								System.out.print("> ");
								String modDepart = sc.next();						
								departments[i] = modDepart;
								System.out.printf("<<<사원정보>>>\n- 사번 : %s\n- 이름 : %s\n- 나이 : %d 세\n- 부서명 : %s\n\n",userNums[i],names[i],ages[i],departments[i]);
								break;
